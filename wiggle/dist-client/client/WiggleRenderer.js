...
export default class WiggleRenderer extends Renderer {
    constructor(gameEngine) {
        ...
        if (width / game.spaceWidth < this.zoom)
            this.zoom = width / game.spaceWidth;
    ...
    draw(t, dt) {
        ...
        // Draw all things
        let wiggles = game.world.queryObjects({ instanceType: Wiggle });
        wiggles.forEach((w) => this.drawWiggle(w));
        let foods = game.world.queryObjects({ instanceType: Food });
    ...
    drawWiggle(w) {
        ...
        let y = w.position.y;
        if (isPlayer)
            ctx.fillStyle = this.rainbowColors();
