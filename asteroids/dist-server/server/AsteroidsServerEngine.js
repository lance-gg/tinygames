...
export default class AsteroidsServerEngine extends ServerEngine {
    constructor(io, gameEngine, inputOptions) {
        ...
        this.asteroidGameEngine = this.gameEngine;
    }
    start() {
        super.start();
    ...
    handleCollision(evt) {
        ...
        this.gameEngine.world.forEachObject((id, obj) => {
            if (obj.physicsObj === evt.bodyA)
                A = obj;
            if (obj.physicsObj === evt.bodyB)
                B = obj;
        });
        ...
        if (A instanceof Bullet && B instanceof Asteroid)
            this.asteroidGameEngine.explode(B, A);
        if (B instanceof Bullet && A instanceof Asteroid)
            this.asteroidGameEngine.explode(A, B);
        if (A instanceof Ship && B instanceof Asteroid)
            this.kill(A);
        if (B instanceof Ship && A instanceof Asteroid)
            this.kill(B);
        // restart game
    ...
    shoot(player) {
        ...
        let bullet = new Bullet(this.gameEngine, {}, {
            mass: 0.05,
            position: new TwoVector(radius * Math.cos(angle) + player.physicsObj.position[0], radius * Math.sin(angle) + player.physicsObj.position[1]),
            velocity: new TwoVector(2 * Math.cos(angle) + player.physicsObj.velocity[0], 2 * Math.sin(angle) + player.physicsObj.velocity[1]),
            angle: 0,
            angularVelocity: 0
        });
    ...
    kill(ship) {
        if (ship.lives-- === 0)
            this.gameEngine.removeObjectFromWorld(ship.id);
    }
